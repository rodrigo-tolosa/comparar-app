...
import { Document, Packer, Paragraph, TextRun, Table, TableRow, TableCell, WidthType, BorderStyle, ShadingType, AlignmentType, ImageRun } from "docx";
  ...
  function rechazos(analisis, mrRows, embudoPng){
    ...
    // Tabla 2: rechazos cualitativos (MR) — la razón sale de la columna Aceptada (Si/No)
    const accVals=["si","sí","s","yes"];
    const rechCual = (mrRows||[]).filter(r=>{ const a=String(r["Aceptada (Si/No)"]!=null?r["Aceptada (Si/No)"]:"").trim(); return a!=="" && !accVals.includes(a.toLowerCase()); });
    children.push(new Paragraph({children:[new TextRun("")]}));
    children.push(new Paragraph({children:[run("EMPRESAS RECHAZADAS POR REVISIÓN CUALITATIVA (MR)",{bold:true,size:12})]}));
    children.push(new Paragraph({children:[new TextRun("")]}));
    if(rechCual.length){
      const rows2=[ new TableRow({children:[hdr("Empresa"),hdr("RIC"),hdr("Razón de Rechazo")]}) ];
      rechCual.forEach(r=>{
        const nombre=limpiarTexto(r["Empresa"]), ric=limpiarTexto(r["RIC"]);
        rows2.push(new TableRow({children:[
          cell(para(nombre||ric,{size:9}),{widthPct:42,borders:bord(null,'single')}),
          cell(para(ric,{size:9}),{widthPct:17,borders:bord(null,'single')}),
          cell(para(limpiarRazon(r["Aceptada (Si/No)"]),{size:9}),{widthPct:41,borders:bord(null,'single')}) ]}));
      });
      children.push(tbl(rows2,{grid:true}));
    } else {
      children.push(para("No hay empresas rechazadas por revisión cualitativa."));
    }
    return doc(children);
  }
